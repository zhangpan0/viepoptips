<!-- -->
<template>
    <section ref="ietips" v-if="showModal">
      <div  class="ietips">
        <h1>以便我们更好的为您提供服务！</h1>
        <p>您正在使用 Internet Explorer 的早期版本（或使用该内核的浏览器）。体验效果会不佳。</p>
        <hr>
        ...
        <ul class="browser">
            <li class="browser-chrome"><a href="https://www.google.cn/chrome/browser/desktop/index.html?hl=zh-CN&standalone=1"> 谷歌浏览器<span> <br>Google Chrome</span></a></li>
            <li class="browser-firefox"><a href="https://www.firefox.com.cn/"> 火狐浏览器<span> <br>Mozilla Firefox</span></a></li>
            ...
            <li class="browser-qq"><a href="https://browser.qq.com/?adtag=SEM170314011"> QQ 浏览器<span> <br> QQ Browser</span></a></li>
            <li class="browser-360"><a href="http://se.360.cn/"> 360安全浏览器<span> <br>360 Chrome</span></a></li>
            <div class="clean"></div>
        </ul>
        <hr>
        <div class="footer color-them-b border-top-1px pointer" @click="showModal = !showModal">
          好的
        </div>
      </div>
      <div
...
export default {
  ...
  methods: {
     myBrowser(Boolean) {
      // Boolean为true时，返回是否是IE浏览器，Boolean为空时返回浏览器类型
  ...
  mounted () {
    this.showModal = this.myBrowser(true);
  }
}
</script>
...
.browser .browser-qq{background-position:0 -136px;margin-left: -27px}
</style>